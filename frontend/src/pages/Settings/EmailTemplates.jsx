...
export default function EmailTemplates() {
  ...
  function refresh() {
    client.get('/settings/email-templates').then((res) => setTemplates(res.data));
  }
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    if (editing) {
      await client.put(`/settings/email-templates/${editing.id}`, form);
    } else {
      await client.post('/settings/email-templates', form);
    }
    setShowModal(false);
    refresh();
  }
  ...
  async function handleDelete(id) {
    await client.delete(`/settings/email-templates/${id}`);
    refresh();
  }
        ...
        <div className="card divide-y divide-gray-100 dark:divide-gray-800">
          {templates.map((t) => (
            <div key={t.id} className="p-4">
              ...
              <div className="text-xs text-gray-400 mt-2 whitespace-pre-wrap">{t.body}</div>
            </div>
          ))}
